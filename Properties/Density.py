from Parameters import MWs_l, MWs_v, R


def liquid_density(Tl, w_MEA, alpha, df_param):

    x_MEA = (1 + alpha + (MWs_l[1] / MWs_l[2]) * ((1 - w_MEA) / w_MEA)) ** (-1)
    x_CO2 = alpha * x_MEA
    x_H2O = 1 - (x_MEA + x_CO2)
    ...
    rho_mol_l = V_l**-1*1.015  # Liquid Molar Density (m3/mol --> mol/m3)
